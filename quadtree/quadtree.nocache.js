...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "quadtree" because\n';
  ...
  $wnd.__gwt_activeModules['quadtree'] = {
    'moduleName' : 'quadtree',
    'bindings' : function() {
      return {};
    }
 ...
 * "/quadtree.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['quadtree'].superdevmode = true;

  var expectedSuffix = '/quadtree.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + quadtree + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:quadtree';
  var devModeSessionKey = '__gwtDevModeSession:quadtree';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/quadtree/quadtree.nocache.js';
  ...
  $wnd.__gwt_activeModules['quadtree'].canRedirect = true;
  $wnd.__gwt_activeModules['quadtree'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/quadtree?user.agent=' + ua + '&_callback=' + callback;
